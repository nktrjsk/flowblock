...
export const LAST_ROUTINE_GEN_KEY = "flowblock_last_routine_gen";
...
async function runRoutineGeneration() {
  ...
  for (const block of allBlocks) {
    ...
    activeBlocksByDate.get(dateStr)!.push({
      start: block.start as string,
      end: block.end as string,
    });
  }

  // External events indexed by `${calendar_id}::${caldav_uid}::${dateStr}`
  ...
  for (const template of templates) {
    ...
    const recurrenceDays: number[] = (() => {
      ...
      if (recurrence === "custom" && template.recurrence_days) {
        try {
          return JSON.parse(template.recurrence_days as string);
        } catch {
          return [];
        }
      }
    ...
    for (let i = 0; i <= GENERATION_WINDOW_DAYS; i++) {
      ...
      const liveKey =
        sourceCalendarId && sourceEventUid
      ...
      if (liveEvent) {
        ...
        // Live link but event not found for this day — skip
        continue;
      } else {
...
export { runRoutineGeneration };

/** Forces routine generation regardless of the daily guard. */
...
export async function deleteFutureBlocksForTemplate(
